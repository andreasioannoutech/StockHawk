...
/**
 * Created by kikkos on 10/26/2016.
 */

public class DummyData {

    // DUMMY DATA BECAUSE OF YAHOO API ISSUE SOME TIMES WHILE WORKING ON THE APP.
    // FOUND THE DATA IN UDACITY FORUMS.

    public static void setDummyDataUsed(Context c,  boolean status){
        String key = c.getString(R.string.dummy_data_used_key);
    ...
    public static boolean getDummyDataUsed(Context c){
        String key = c.getString(R.string.dummy_data_used_key);
        SharedPreferences settings = PreferenceManager.getDefaultSharedPreferences(c);
        return settings.getBoolean(key, false);
    }

    public static final String yahooApiResponseStr = "{\n" +
            " \"query\": {\n" +
            "  \"count\": 4,\n" +
